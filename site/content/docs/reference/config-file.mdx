...
> **Not to be confused with `.straitjacket.toml`.** The YAML file here sets *run
> settings*. A [`.straitjacket.toml`](/docs/guides/monorepos) is a separate
> monorepo *project marker* — its presence draws a boundary the cross-file rules
> don't compare across. Different file, different job.
